...
import { useStore, type RemoteShareState } from '../store/useStore'
...
export function RemoteBanner() {
  ...
  const handleFork = useCallback(() => {
    ...
    // Snapshot the current banner payload so "Undo" can restore it if the
    // user clicks by accident. Fork is soft-reversible until the banner
    // fully clears (≤4s), not destructive; any imported rows persist.
    const snapshot: RemoteShareState = { ...remoteShare, forked: false }
    setRemoteShare({ ...remoteShare, forked: true })
    setTimeout(() => setRemoteShare(null), 600)
    showToast('Forked — edits stay local', {
      durationMs: 4000,
      actionLabel: 'Undo',
      onAction: () => setRemoteShare(snapshot),
    })
  ...
  const hl = remoteShare.highlightCount.toLocaleString()
  const cm = remoteShare.commentCount.toLocaleString()
  ...
  return (
    ...
    >
      ...
      <span className="text-blue-700 dark:text-blue-300">
        · {hl} highlight{remoteShare.highlightCount === 1 ? '' : 's'}
        {', '}{cm} comment{remoteShare.commentCount === 1 ? '' : 's'}
      </span>
      {remoteShare.driftWarning && (
        <span className="inline-flex items-center gap-1 text-amber-700 dark:text-amber-400">
          ...
          <button
            ...
            title="Open the diff to see exactly what changed"
          >
            show diff
          </button>
        </span>
