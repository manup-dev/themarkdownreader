...
describe('runPipeline', () => {
  it('writes a companion file for each .md+.annot pair and reports the changed paths', async () => {
    ...
    const event = JSON.stringify({
      ...
      selectedText: 'body line', body: 'note', author: 'manu', sectionId: '',
    }) + '\n'
  ...
  it('removes a stale companion when all comments are deleted', async () => {
    ...
    await writeFile(join(root, 'doc.md.comments.md'), '<!-- generated by md-reader github-integration · do not edit -->\nstale stuff\n')
  ...
  it.skipIf(isRoot)('isolates a single corrupt sidecar and continues with the rest', async () => {
    ...
    await writeFile(join(root, '.good.md.annot'),
      JSON.stringify({ v:1, ts: Date.UTC(2026,3,30), id:'c1', op:'comment.add',
        ...
        selectedText:'body', body:'note', author:'a', sectionId:'' }) + '\n')
