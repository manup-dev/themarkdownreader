/**
 * Vendored copy of ../../src/lib/annotation-events.ts for the github-integration
 * Node package. The two `import type` statements from the parent (anchor.ts and
 * docstore.ts) have been replaced with inline interface definitions so that this
 * package does not drag in DOM-dependent or browser-only parent source files
 * during typecheck.
 *
 * KEEP IN SYNC with the parent file. Types inlined here:
 *   - TextAnchor  (from src/lib/anchor.ts)
 *   - Highlight   (from src/lib/docstore.ts)
 *   - Comment     (from src/lib/docstore.ts)
 */

// ─── Inlined from src/lib/anchor.ts ─────────────────────────────────────────
...
// ─── Inlined from src/lib/docstore.ts ───────────────────────────────────────
